...
namespace OpenFAST.UnitTests.Template.Operators
{
    ...
    public class DeltaStringOperatorTest : OpenFastTestCase
    {
        ...
        {
            _field = new Scalar("", FastType.Ascii, Operator.Delta, ScalarValue.Undefined, true);
            Assert.AreEqual(Tv(0, "ABCD"), Encode("ABCD", ScalarValue.Undefined));
            Assert.AreEqual(Tv(1, "E"), Encode("ABCE", new StringValue("ABCD")));
            Assert.AreEqual(Tv(-2, "Z"), Encode("ZBCE", new StringValue("ABCE")));
            Assert.AreEqual(Tv(-1, "Y"), Encode("YZBCE", new StringValue("ZBCE")));
        ...
        private ScalarValue Encode(string value, ScalarValue priorValue)
        {
            if (value == null)
            {
                return OpenFAST.Template.Operators.OperatorCodec.GetCodec(Operator.Delta, FastType.String).GetValueToEncode(null, priorValue, _field);
            }
            return OpenFAST.Template.Operators.OperatorCodec.GetCodec(Operator.Delta, FastType.String).GetValueToEncode(new StringValue(value), priorValue, _field);
        }

        private ScalarValue Decode(ScalarValue diff, ScalarValue priorValue)
        {
            return OpenFAST.Template.Operators.OperatorCodec.GetCodec(Operator.Delta, FastType.String).DecodeValue(diff, priorValue, _field);
        }
