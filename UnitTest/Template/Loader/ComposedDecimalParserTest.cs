...
namespace OpenFAST.UnitTests.Template.Loader
{
    ...
    public class ComposedDecimalParserTest : OpenFastTestCase
    {
        ...
        {
            XmlElement decimalDef = Document("<decimal name=\"composed\"><mantissa><delta/></mantissa><exponent><constant value=\"-2\"/></exponent></decimal>").DocumentElement;
            Assert.True(_parser.CanParse(decimalDef, _context));
            var decimalt = (ComposedScalar)_parser.Parse(decimalDef, _context);
            AssertComposedScalarField(decimalt, FASTType.Decimal, "composed", OpenFAST.Template.Operator.Operator.Constant, new IntegerValue(-2), OpenFAST.Template.Operator.Operator.Delta, ScalarValue.Undefined);
        }
        [Test]
        ...
        {
            XmlElement decimalDef = Document("<decimal name=\"composed\"><mantissa><delta dictionary=\"template\" key=\"variable\" value=\"100\"/></mantissa><exponent><copy dictionary=\"template\" key=\"static\" value=\"-2\"/></exponent></decimal>").DocumentElement;
            Assert.True(_parser.CanParse(decimalDef, _context));
            var decimalt = (ComposedScalar)_parser.Parse(decimalDef, _context);

            AssertComposedScalarField(decimalt, FASTType.Decimal, "composed", OpenFAST.Template.Operator.Operator.Copy, new IntegerValue(-2), OpenFAST.Template.Operator.Operator.Delta, new IntegerValue(100));
