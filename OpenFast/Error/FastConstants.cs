...
namespace OpenFAST.Error
{
    ...
    {
        public static readonly QName ANY_TYPE;
        public static readonly FastAlertSeverity ERROR;
        public static readonly FastAlertSeverity WARN;
        public static readonly FastAlertSeverity FATAL;
        // Error Types
        public static readonly ErrorType DYNAMIC;
        public static readonly ErrorType STATIC;
        public static readonly ErrorType REPORTABLE;
        // Static Errors
        ...
        public static readonly string TEMPLATE_DEFINITION_1_1 = "http://www.fixprotocol.org/ns/fast/td/1.1";
        ...
        static FastConstants()
        {
            ...
            S4_NO_INITIAL_VALUE_FOR_CONST = new ErrorCode(STATIC, 4, "ERR S4",
                                                          "Fields with constant operators must have a default value defined.",
                                                          ERROR);
            S5_NO_INITVAL_MNDTRY_DFALT = new ErrorCode(STATIC, 5, "ERR S5",
                                                       "No initial value for mandatory field with default operator",
                                                       ERROR);
            D1_FIELD_APP_INCOMP = new ErrorCode(DYNAMIC, 1, "ERR D1",
                                                "Field cannot be converted to type of application field", ERROR);
            D2_INT_OUT_OF_RANGE = new ErrorCode(DYNAMIC, 2, "ERR D2",
                                                "The integer value is out of range for the specified integer type.",
                                                WARN);
            D3_CANT_ENCODE_VALUE = new ErrorCode(DYNAMIC, 3, "ERR D3",
                                                 "The value cannot be encoded for the given operator.", ERROR);
            D4_INVALID_TYPE = new ErrorCode(DYNAMIC, 4, "ERR D4",
                                            "The previous value is not the same type as the type of the current field.",
                                            ERROR);
            D5_NO_DEFAULT_VALUE = new ErrorCode(DYNAMIC, 5, "ERR D5",
                                                "If no prior value is set and the field is not present, there must be a default value or the optional flag must be set.",
                                                ERROR);
            D6_MNDTRY_FIELD_NOT_PRESENT = new ErrorCode(DYNAMIC, 6, "ERR D6", "A mandatory field must have a value",
                                                        ERROR);
            D7_SUBTRCTN_LEN_LONG = new ErrorCode(DYNAMIC, 7, "ERR D7",
            ...
            D9_TEMPLATE_NOT_REGISTERED = new ErrorCode(DYNAMIC, 9, "ERR D9", "The template has not been registered.",
                                                       ERROR);
            R1_LARGE_DECIMAL = new ErrorCode(REPORTABLE, 1, "ERR R1",
            ...
            R5_DECIMAL_CANT_CONVERT_TO_INT = new ErrorCode(REPORTABLE, 5, "ERR R5",
